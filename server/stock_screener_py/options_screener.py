...
class Options_screener:
    ...
    async def getOptions(self):
        ...
        while len(self.tickers) > 0:
            ...
            input_coroutines = list(map(lambda ticker: asyncio.ensure_future(self.addOption(ticker)), self.tickers[:50]))
            await asyncio.gather(*input_coroutines, return_exceptions=False)
        return self.options

    def convertOptionsAndpush(self, ticker, options):
        self.tickers.remove(ticker)
        print("saving options for {} options size: {}, proxies size: {}, tickers size: {}".format(ticker, len(self.options), len(self.proxies), len(self.tickers)))
    ...
    async def addOption(self, ticker):
        ...
        try:
            ...
            try:
                ...
                logging.debug("removed proxy: {}. Count: {}".format(proxy, len(self.proxies)))
            except:
